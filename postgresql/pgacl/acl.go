...
SCAN:
	for i = idx + 1; i < aclLen; i++ {
		...
		case 's':
			acl.Privileges |= Set
			if withGrant() {
				acl.GrantOptions |= Set
			}
		case 'A':
			acl.Privileges |= System
			if withGrant() {
				acl.GrantOptions |= System
			}
		case 'm':
			acl.Privileges |= Maintain
			if withGrant() {
				acl.GrantOptions |= Maintain
			}
		case '/':
			if i+1 <= aclLen {
